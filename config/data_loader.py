...
def build_time_features(stamp: pd.DataFrame, freq: str, timeenc: int):
    stamp = stamp.copy()
    ...
    if timeenc == 0:
        ...
        return stamp.drop(["date"], axis=1).values
...
class DatasetETTHourMulti(Dataset):
    ...
    def _read_data(self) -> None:
        ...
        data_stamp = build_time_features(
            stamp=raw[["date"]][border1:border2],
            freq=self.freq,
...
class DatasetCustomMulti(Dataset):
    def __init__(
        self,
        root_path: str,
        flag: str = "train",
    ...
    def _read_data(self) -> None:
        ...
        if self.target in columns:
            ...
            raw = raw[["date"] + columns + [self.target]]
        else:
            self.target = columns[-1]

        train_count = int(len(raw) * 0.7)
        test_count = int(len(raw) * 0.2)
        val_count = len(raw) - train_count - test_count
        border1s = [
            0,
            train_count - self.seq_len,
            len(raw) - test_count - self.seq_len,
        ]
        border2s = [
            train_count,
